...
imagefile = 'tennis_balls.jpg';
ntilesout = [5 10];   % how many tiles wide and tall the output should be
tilesize = 35;    % the size of the tiles we paste down (assumed square)
...
for i = 1:ntilesout(1)
  for j = 1:ntilesout(2)
    ...
    elseif (j==1) %first column (but not first row)
        ...
        strip = [stripR;stripB]; %concatenate the two together
        indLT = [indL;indT];

        % compare to the top and left of all our candidates
